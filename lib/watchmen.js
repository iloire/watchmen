...
WatchMen.prototype.ping = function (params, callback){
  ...
  params.service.ping_service.ping (params.service, function(error, body, response, elapsed_time){
    ...
    self.storage.get_status (params.service, function (err, prev_state) {
      ...
      if (state.uptime > 100 || state.uptime < 0) {
        console.log(state);
        throw 'Error calculating uptime:' + state.uptime + ' for service: ' + params.service.url_info + '. Are you running more than watchmen monitor simultaneously?';
      }
      ...
      if (error) { // service is down
        ...
        if (prev_state.status === "error") { //service was down and now it is up again!
          ...
          add_event('success', 'service is back! Downtime: ' + moment.duration(state.down_time_last_request,'seconds').humanize(), 'critical');
