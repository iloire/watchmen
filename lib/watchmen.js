...
function WatchMen(services, storage, ping_service, options){
...
-----------------*/
WatchMen.prototype._validate_http_response = function (service, body, res){
  if (service.expected){
...
WatchMen.prototype.ping = function (params, callback){
  ...
  self.ping_service.ping (params.service, function(request_err, body, response, elapsed_time){
    ...
    self.storage.get_status (params.service, function (err, prev_state){
      ...
      //-------------------------------------
      state.error = request_err ? ('Connection error: ' + request_err)
          : self._validate_http_response(params.service, body, response);
      ...
      //-------------------------------------
        ...
        if (limit && (elapsed_time > limit)){ //over the limit. warning!
          ...
          add_event('warning', elapsed_time + ' ms (limit: ' + limit + ' ms) ## ' + state.warnings + ' warnings', 'warning');
