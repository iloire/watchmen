...
NotificationService.prototype.sendServiceDownAlert = function(service, state, cb) {
    this._send({
        ...
        title: EMAIL_SUBJECT_PREFIX + ' ' + service.url_info + ' is down!',
        body: service.url_info + ' is down!. Reason: ' + state.error
...
NotificationService.prototype.sendServiceBackAlert = function(service, state, cb) {
    this._send({
        to: this._getRecipients(service),
        title: EMAIL_SUBJECT_PREFIX + ' ' + service.url_info + ' is back!',
        body: service.url_info + ' is back. Down for ' + state.down_time_last_request + ' seconds'
    }, cb || consoleOutputHandler);
};
