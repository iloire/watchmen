 ...
 */
var defaultConfig = require('../../../config/notification-services/aws-ses');
...
AWSSes.prototype.send = function(options, cb) {
    ...
    if (!this.config.from || !this.config.region || !this.config.AWS_KEY || !this.config.AWS_SECRET){
        return cb('invalid configuration for service AWS-SES. Please fix it in config/notification-services/aws-ses');
    }
