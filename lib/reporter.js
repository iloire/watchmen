...
/**
 * TODO: handle auth in a layer above this
 *
...
Reporter.prototype.getService = function(serviceId, callback){
  ...
  storage.getService(serviceId, function(err, service){
    ...
    getGeneralServiceInfo(service, storage, function(err, serviceData){
      ...
      storage.getOutagesSince(service, lastWeek, function(err, outagesLastWeek){
        if (err) {
          return callback(err);
        }

        var uptimeInfoLastWeek = getUptime(service, outagesLastWeek,
          serviceData.status.currentOutage, lastWeek);

        var uptimeInfoLastHour = getUptime(service, outagesLastWeek,
        ...
        storage.getLatencySince(serviceData.service, +new Date() - WEEK, 'day', function(err, latencyLastWeek){
          ...
          if (err) {
            ...
            storage.getLatencySince(serviceData.service, +new Date() - HOUR, 'minute', function(err, latencyLastHour) {
              ...
              serviceData.status.lastWeek = {};
              serviceData.status.lastWeek.outages = outagesLastWeek;
              serviceData.status.lastWeek.latency = latencyLastWeek;
              serviceData.status.lastWeek.uptime = uptimeInfoLastWeek.uptime;

              serviceData.status.lastHour = {};
              serviceData.status.lastHour.latency = latencyLastHour;
              serviceData.status.lastHour.uptime = uptimeInfoLastHour.uptime;

              // rest of the properties for the last 24h are populated in getGeneralServiceInfo
...
function getUptime (service, outages, currentOutage, since) {
  ...
  var uptime = utils.round((totalTime - totalDownTime) * 100 / totalTime);
...
function getGeneralServiceInfo(service, storage, callback) {
  ...
  storage.getCurrentOutage(service, function(err, currentOutage){
    ...
    storage.getOutagesSince(service, last24H, function(err, outages){
      if (err) {
        return callback(err);
      }

      var uptimeInfoLast24H = getUptime(service, outages, currentOutage, last24H);
      ...
      return callback(null, {
        ...
        status: {
          ...
          last24Hours: {
            outages: outages,
            numberOutages: outages.length,
            downtime: uptimeInfoLast24H.totalDownTime,
            uptime: uptimeInfoLast24H.uptime
          }
        }
      });
    });
