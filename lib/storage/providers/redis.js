...
StorageRedis.prototype.archiveCurrentOutageIfExists = function (service, callback) {
  var self = this;
  self.resetOutageFailureCount(service, function (err) {

    if (err) {
      return callback(err);
    }

    self.getCurrentOutage(service, function (err, outage) {
      if (err) {
        return callback(err);
      }
...
StorageRedis.prototype.getLatencySince = function (service, timestamp, aggregatedBy, callback) {
  this.redis.zrevrangebyscore(service.id + ':' + LATENCY_KEY_SUFIX, '+inf', timestamp || '-inf', 'withscores', function (err, data) {
    ...
    var data = parseLatencyDataFromZset(data);
    if (aggregatedBy) {
      aggregator.aggregate(data.arr, aggregatedBy, function (aggregatedData) {
        callback(null, {
          list: aggregatedData,
          mean: data.mean
        });
...
StorageRedis.prototype.resetOutageFailureCount = function (service, cb) {
  this.redis.del(service.id + ':failurecount', cb);
};

StorageRedis.prototype.increaseOutageFailureCount = function (service, cb) {
  this.redis.incr(service.id + ':failurecount', cb);
};
